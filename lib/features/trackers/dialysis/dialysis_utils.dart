...
class DialysisUtils {
  ...
  static double? calculateUFRate(DialysisModel model) {
    if (model.ultrafiltration == null || model.duration <= 0) {
      return null;
    }
    return model.ultrafiltration!.volume / model.duration;
  }
  ...
  static bool isCompleteSession(DialysisModel model) {
    return model.preWeight != null &&
        ...
        isValidDuration(type: model.type, duration: model.duration);
  }
  ...
  static int getSessionCompleteness(DialysisModel model) {
    ...
    if (model.ultrafiltration != null) completedFields++;
    if (isValidDuration(type: model.type, duration: model.duration)) {
      completedFields++;
    }
    if (model.notes != null && model.notes!.isNotEmpty) completedFields++;
  ...
  static Map<DialysisType, List<DialysisModel>> groupByType(
    ...
    for (final session in sessions) {
      groups.putIfAbsent(session.type, () => []).add(session);
    }
  ...
  static double? getAverageSessionDuration(
    ...
    final totalDuration =
        typeSessions.fold<double>(0, (sum, s) => sum + s.duration);
  ...
  static bool isOptimalDuration(DialysisModel model) {
    final optimal = DialysisConstants.getOptimalDuration(model.type.name);
    return model.duration >= optimal;
  }

  /// Generates a summary string for a dialysis session
  static String generateSessionSummary(DialysisModel model) {
    final buffer = StringBuffer();
    buffer.write('${model.type.displayName} - ${model.duration} hrs');
  ...
  static double getTotalDialysisTime(List<DialysisModel> sessions) {
    return sessions.fold<double>(0, (sum, session) => sum + session.duration);
  }
}
